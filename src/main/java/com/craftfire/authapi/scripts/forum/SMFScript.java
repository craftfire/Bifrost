...
public class SMFScript extends Script {
    ...
    public boolean authenticate(String username, String password) {
        String passwordHash = this.dataManager.getStringField("SELECT `passwd` FROM `" + this.dataManager.getPrefix() + "members` WHERE `" + this.membernamefield + "` = '" + username + "'");
	...
	public ScriptUser getLastRegUser() {
		return getUser(this.dataManager.getIntegerField("SELECT `id_member` FROM `" + this.dataManager.getPrefix() + "members` ORDER BY `id_member` ASC LIMIT 1"));
	}
	...
	public ScriptUser getUser(int userid) {
        JTable userTable = new JTable(this.dataManager.resultSetToTableModel("SELECT * FROM `" + this.dataManager.getPrefix() + "members` WHERE `id_member` = '" + userid + "' LIMIT 1"));
        String title = null, savedusername = null, nickname = null, email = null, password = null, passwordsalt = null, avatarurl = null, regip = null, lastip = null;
	...
	public void createUser(ScriptUser user) {
		...
		this.dataManager.executeSQLquery("UPDATE `" + this.dataManager.getPrefix() + "settings" + "` SET `value` =" +
										 " '" + (user.getRegDate().getTime() / 1000) + "' WHERE `variable` = 'memberlist_updated'");
    ...
    public String getUsername(int userid) {
        return this.dataManager.getStringField("SELECT `" + this.membernamefield + "` FROM `" + this.dataManager.getPrefix() + "members` WHERE `id_member` = '" + userid + "'");
    }

    public int getUserID(String username) {
        return this.dataManager.getIntegerField("SELECT `id_member` FROM `" + this.dataManager.getPrefix() + "members` WHERE `" + this.membernamefield + "` = '" + username + "'");
    }
    ...
    public List<Group> getGroups(int limit) {
        ...
        JTable groupTable = new JTable(this.dataManager.resultSetToTableModel("SELECT `id_group` FROM `" + this.dataManager.getPrefix() + "membergroups` ORDER BY `id_group` ASC" + limitstring));
	...
	public Group getGroup(int groupid) {
		JTable groupTable = new JTable(this.dataManager.resultSetToTableModel("SELECT * FROM `" + this.dataManager.getPrefix() + "membergroups` WHERE `id_group` = '" + groupid + "' ORDER BY `id_group` ASC LIMIT 1"));
		...
		if (groupTable.getRowCount() == 1) {
			...
			JTable userTable = new JTable(this.dataManager.resultSetToTableModel("SELECT `" + this.membernamefield + "` FROM `" + this.dataManager.getPrefix() + "members` WHERE `id_group` = '" + groupid + "' ORDER BY `id_member` ASC"));
		...
		group.setUserCount(this.dataManager.getIntegerField("SELECT COUNT(*) FROM `" + this.dataManager.getPrefix() + "members` WHERE `id_group` = '" + groupid + "'"));
	...
	public List<Group> getUserGroups(String username) {
		...
		JTable userTable = new JTable(this.dataManager.resultSetToTableModel("SELECT `id_group`, `" + this.groupfield + "`  FROM `" + this.dataManager.getPrefix() + "members` WHERE `" + this.membernamefield + "` = '" + username + "' ORDER BY `id_member` ASC LIMIT 1"));
		List<Group> groups = new ArrayList<Group>();
		if (userTable.getRowCount() == 1) {
	...
	public List<PrivateMessage> getPMsSent(String username, int limit) {
		...
		JTable pmTable = new JTable(this.dataManager.resultSetToTableModel("SELECT * FROM `" + this.dataManager.getPrefix() + "personal_messages` WHERE `id_member_from` = '" + getUserID(username) + "' ORDER BY `id_pm` ASC" + limitstring));
		...
		for (int i=0; pmTable.getRowCount() > i; i++) {
			...
			if (pmTable.getRowCount() == 1) {
				...
				JTable pmeTable = new JTable(this.dataManager.resultSetToTableModel("SELECT * FROM `" + this.dataManager.getPrefix() + "pm_recipients` WHERE `id_pm` = '" + pmid + "' LIMIT 1"));
			...
			pm.setFromUser(getUser(fromuserid));
	...
	public List<PrivateMessage> getPMsReceived(String username, int limit) {
		...
		JTable pmTable = new JTable(this.dataManager.resultSetToTableModel("SELECT * FROM `" + this.dataManager.getPrefix() + "pm_recipients` WHERE `id_member` = '" + getUserID(username) + "' ORDER BY `id_pm` ASC" + limitstring));
		List<PrivateMessage> pms = new ArrayList<PrivateMessage>();
		for (int i=0; pmTable.getRowCount() > i; i++) {
			int pmid = 0, fromuserid = 0, touserid = 0;
			boolean deletedbysender = false, isread = false, isnew = false, isdeleted = false;
			...
			if (pmTable.getRowCount() == 1) {
				...
				JTable pmeTable = new JTable(this.dataManager.resultSetToTableModel("SELECT * FROM `" + this.dataManager.getPrefix() + "personal_messages` WHERE `id_pm` = '" + pmid + "' LIMIT 1"));
				if (pmeTable.getRowCount() == 1) {
	...
	public int getPMSentCount(String username) {
		return this.dataManager.getIntegerField("SELECT COUNT(*) FROM `" + this.dataManager.getPrefix() + "personal_messages` WHERE `id_member_from` = '" + getUserID(username) + "'");
	}

	public int getPMReceivedCount(String username) {
		return this.dataManager.getIntegerField("SELECT COUNT(*) FROM `" + this.dataManager.getPrefix() + "pm_recipients` WHERE `id_member` = '" + getUserID(username) + "'");
	}

	public void updatePrivateMessage(PrivateMessage pm) {
		String temp;
		HashMap<String, Object> data = new HashMap<String, Object>();
		data.put("id_member_from", pm.getFromUser().getID());
		...
		if (CraftCommons.inVersionRange(this.versionRanges[0], this.userVersion)) {
			...
			data.put("fromname", pm.getFromUser().getUsername());
		} else if (CraftCommons.inVersionRange(this.versionRanges[1], this.userVersion)) {
			data.put("deleted_by_sender", temp);
			data.put("from_name", pm.getFromUser().getUsername());
		}
    ...
    public int getPostCount(String username) {
        return this.dataManager.getIntegerField("SELECT `posts` FROM `" + this.dataManager.getPrefix() + "members` WHERE `" + this.membernamefield + "` = '" + username + "'");
    }
	
	public Post getPost(int postid) {
		JTable postTable = new JTable(this.dataManager.resultSetToTableModel("SELECT * FROM `" + this.dataManager.getPrefix() + "messages` WHERE `id_msg` = '" + postid + "' LIMIT 1"));
    ...
    public Post getLastUserPost(String username) {
        ...
        return getPost(this.dataManager.getIntegerField("SELECT `id_msg` FROM `" + this.dataManager.getPrefix() + "messages` WHERE `id_member` = '" + userid + "' ORDER BY `id_msg` ASC LIMIT 1"));
    }

    public Post getLastPost() {
		return getPost(this.dataManager.getIntegerField("SELECT `id_msg` FROM `" + this.dataManager.getPrefix() + "messages` ORDER BY `id_msg` ASC LIMIT 1"));
	}
    ...
    public List<Post> getPosts(int limit) {
        ...
        JTable postTable = new JTable(this.dataManager.resultSetToTableModel("SELECT `id_msg` FROM `" + this.dataManager.getPrefix() + "messages` ORDER BY `id_msg` ASC" + limitstring));
	...
	public List<Post> getPostsFromThread(int threadid, int limit) {
		...
		JTable postTable = new JTable(this.dataManager.resultSetToTableModel("SELECT `id_msg` FROM `" + this.dataManager.getPrefix() + "messages` WHERE `id_topic` = '" + threadid + "' ORDER BY `id_msg` ASC" + limitstring));
		List<Post> posts = new ArrayList<Post>();
		for (int i=0; postTable.getRowCount() > i; i++) {
			posts.add(getPost(Integer.parseInt(postTable.getModel().getValueAt(0, 0).toString())));
		}
	...
	public void createPost(Post post) {
		...
		if (CraftCommons.inVersionRange(this.versionRanges[0], this.userVersion)) {
			this.dataManager.executeSQLquery("UPDATE `" + this.dataManager.getPrefix() + "topics" + "` SET `numreplies` =" +
											 " numreplies + 1 WHERE `id_topic` = '" + post.getThreadID() + "'");
		} else if (CraftCommons.inVersionRange(this.versionRanges[1], this.userVersion)) {
			this.dataManager.executeSQLquery("UPDATE `" + this.dataManager.getPrefix() + "topics" + "` SET `num_replies` =" +
		...
		this.dataManager.executeSQLquery("UPDATE `" + this.dataManager.getPrefix() + "boards" + "` SET `id_last_msg` =" +
										 " '" +  post.getID() + "', `id_msg_updated` = '" + post.getID() + "' WHERE `id_board` = '" + post.getBoardID() + "'");
		if (CraftCommons.inVersionRange(this.versionRanges[0], this.userVersion)) {
			this.dataManager.executeSQLquery("UPDATE `" + this.dataManager.getPrefix() + "boards" + "` SET `numposts` =" +
											 " numposts + 1 WHERE `id_board` = '" + post.getBoardID() + "'");
		} else if (CraftCommons.inVersionRange(this.versionRanges[1], this.userVersion)) {
			this.dataManager.executeSQLquery("UPDATE `" + this.dataManager.getPrefix() + "boards" + "` SET `num_posts` =" +
    ...
    public int getThreadCount(String username) {
        int userid = getUserID(username);
        return this.dataManager.getIntegerField("SELECT COUNT(*) FROM `" + this.dataManager.getPrefix() + "topics` WHERE `id_member_started` = '" + userid + "'");
    }

    public Thread getLastThread() {
		return getThread(this.dataManager.getIntegerField("SELECT `id_topic` FROM `" + this.dataManager.getPrefix() + "topics` ORDER BY `id_topic` ASC LIMIT 1"));
    }

    public Thread getLastUserThread(String username) {
		int userid = getUserID(username);
		return getThread(this.dataManager.getIntegerField("SELECT `id_topic` FROM `" + this.dataManager.getPrefix() + "topics` WHERE `id_member_started` = '" + userid + "' ORDER BY `id_topic` ASC LIMIT 1"));
    }

	public Thread getThread(int threadid) {
		JTable threadTable = new JTable(this.dataManager.resultSetToTableModel("SELECT * FROM `" + this.dataManager.getPrefix() + "topics` WHERE `id_topic` = '" + threadid + "' LIMIT 1"));
		...
		if (threadTable.getRowCount() == 1) {
			...
			JTable postTable = new JTable(this.dataManager.resultSetToTableModel("SELECT * FROM `" + this.dataManager.getPrefix() + "messages` WHERE `id_msg` = '" + firstpostid + "' LIMIT 1"));
			if (postTable.getRowCount() == 1) {
    ...
    public List<Thread> getThreads(int limit) {
		...
		JTable postTable = new JTable(this.dataManager.resultSetToTableModel("SELECT `id_topic` FROM `" + this.dataManager.getPrefix() + "topics` ORDER BY `id_topic` ASC" + limitstring));
	...
	public void createThread(Thread thread) {
		...
		if (CraftCommons.inVersionRange(this.versionRanges[0], this.userVersion)) {
			this.dataManager.executeSQLquery("UPDATE `" + this.dataManager.getPrefix() + "boards" + "` SET `numtopics` =" +
											 " numtopics + 1 WHERE `id_board` = '" + post.getBoardID() + "'");
			this.dataManager.executeSQLquery("UPDATE `" + this.dataManager.getPrefix() + "topics" + "` SET `numreplies` =" +
											 " '0' WHERE `id_topic` = '" + post.getThreadID() + "'");
		} else if (CraftCommons.inVersionRange(this.versionRanges[1], this.userVersion)) {
			this.dataManager.executeSQLquery("UPDATE `" + this.dataManager.getPrefix() + "boards" + "` SET `num_topics` =" +
    ...
    public int getGroupCount() {
        return this.dataManager.getIntegerField("SELECT COUNT(*) FROM `" + this.dataManager.getPrefix() + "membergroups`");
    }
    ...
    public List<Ban> getBans(int limit) {
        ...
        JTable banTable = new JTable(this.dataManager.resultSetToTableModel("SELECT * FROM `" + this.dataManager.getPrefix() + "ban_groups`" + limitstring));
        ...
        for(int i=0; banTable.getRowCount() > i; i++) {
            ...
            String email = this.dataManager.getStringField("SELECT `email_address` FROM `" + this.dataManager.getPrefix() + "ban_items` WHERE `id_ban_group` = '" + banid + "' AND `email_address` != '' ORDER BY `id_ban` ASC LIMIT 1");
            JTable banIPtable = new JTable(this.dataManager.resultSetToTableModel("SELECT `ip_low1`, `ip_high1`, `ip_low2`, `ip_high2`, `ip_low3`, `ip_high3`, `ip_low4`, `ip_high4` FROM `" + this.dataManager.getPrefix() + "ban_items` WHERE `id_ban_group` = '" + banid + "' AND `ip_low1` != '0' ORDER BY `id_ban` ASC LIMIT 1"));
            String ip = null;
            ...
            int userid = this.dataManager.getIntegerField("SELECT `id_member` FROM `" + this.dataManager.getPrefix() + "ban_items` WHERE `id_ban_group` = '" + banid + "' AND `id_member` != '0' ORDER BY `id_ban` ASC LIMIT 1");
	...
	public void updateBan(Ban ban) {
		...
		if (ban.getIP() != null && !ban.getIP().isEmpty()) {
			...
			this.dataManager.updateFields(data, "ban_items", "`id_ban_group` = '" + ban.getID() + "' AND `ip_low1` != '0'");
		}
		if (ban.getEmail() != null && !ban.getEmail().isEmpty()) {
			data = new HashMap<String, Object>();
			data.put("email_address", ban.getEmail());
			this.dataManager.updateFields(data, "ban_items", "`id_ban_group` = '" + ban.getID() + "' AND `email_address` != ''");
		}
		if (ban.getUserID() > 0) {
			data = new HashMap<String, Object>();
			data.put("id_member", ban.getUserID());
			this.dataManager.updateFields(data, "ban_items", "`id_ban_group` = '" + ban.getID() + "' AND `id_member` != '0'");
		}
	}
	...
	public void addBan(Ban ban) {
		...
		this.dataManager.executeSQLquery("UPDATE `" + this.dataManager.getPrefix() + "settings" + "` SET `value` =" +
										 " '" + (ban.getStartDate().getTime() / 1000) + "' WHERE `variable` = 'banLastUpdated'");
	}
    ...
    public boolean isRegistered(String username) {
        if (this.dataManager.getStringField("SELECT `" + this.membernamefield + "` FROM `" + this.dataManager.getPrefix() + "members` WHERE `" + this.membernamefield + "` = '" + username + "'") == null) {
            return true;
        }
        return false;
    }
